...
aws s3api create-bucket --bucket terraweek-state-aditya-2026 --region ap-south-1 --create-bucket-configuration LocationConstraint=ap-south-1

# Enable versioning (so you can recover previous state)
aws s3api put-bucket-versioning --bucket terraweek-state-aditya-2026 --versioning-configuration Status=Enabled
...
- Add `day-64-state-management.md` to `2026/day-64/`
...
> "Mastered Terraform state today — migrated to S3 remote backend with DynamoDB locking, imported existing AWS resources, performed state surgery, and simulated drift. State management is the foundation of reliable infrastructure as code."
